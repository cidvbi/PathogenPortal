...
class Configuration( object ):
    ...
    def is_power_user( self, user):
        """
        Determine if its a power user
        NOTE: This will need to be changed when the above admin function is changed
        """
        power_users = [x.strip() for x in self.get( "power_users", "" ).split( "," ) ]
        return ( user is not None and power_users and user.email in power_users )
