...
def read_dbnames(filename):
    ...
    try:
        ...
        #log.debug("ERROR: Unable to read builds file: %s"% e)
        pass
    if len(db_names)<1:
...
def read_build_sites( filename, check_builds=True ):
    ...
    try:
        ...
        #log.debug("ERROR: Unable to read builds for site file %s" %filename)
        pass
    return build_sites
