        ...
        def decorator( self, trans, *args, **kwargs ):
            if trans.get_user():
                ...
                return trans.show_error_message(
                    'Please <a target="galaxy_main" href="%s">Log In</a> or <a target="galaxy_main" href="%s">Register</a> %s.'
                    % ( url_for( controller='user', action='login', webapp=webapp), url_for( controller='user', action='create', webapp=webapp), verb ), use_panels=use_panels )
        return decorator
    return argcatcher
...
class GalaxyWebTransaction( base.DefaultWebTransaction ):
    ...
    def user_is_gateway( self, target_user):
        gateway_users=self.app.config.get( "gateway_users", "").split(",")
	return self.user and gateway_users and target_user.email in gateway_users
    def user_is_power(self, target_user):
        power_users=self.app.config.get( "power_users", "").split(",")
        return self.user and power_users and target_user.email in power_users
