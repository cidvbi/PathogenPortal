...
class SelectField(BaseField):
    ...
    def __init__( self, name, multiple=None, display=None, refresh_on_change=False, refresh_on_change_values=None, size=None):
        ...
        elif display =="on_deck":
            pass
        elif display is not None and ("ajax:" in display or "coordinate:" in display):
            pass
    ...
    def get_html( self, prefix="", disabled=False ):
        ...
        elif self.display == "on_deck":
            return self.get_html_deck( prefix, disabled )
        elif self.display is not None and ("ajax:" in self.display or "coordinate:" in self.display):
            return self.get_html_special( prefix, disabled )
        else:
    ...
    def get_html_radio( self, prefix="", disabled=False ):
        ...
        for text, value, selected in self.options:
            ...
            ctr += 1
        return unicodify( "\n".join( rval ) )
    def get_html_deck( self, prefix="", disabled=False ):  
        multiple = " multiple"
        size = ''
        interaction_type= ' interaction_type="on_deck"'
        rval = []
        rval.append('<table border="0">')
        rval.append('<tr>')
        rval.append('<th style="float:left">Available</th>')
        rval.append('<th></th>')
        rval.append('<th style="float:left">Selected</th>')
        rval.append('</tr>')
        rval.append('<tr>')
        rval.append('<td style="vertical-align:top">')
        last_selected_value = ""
        cur_spot=len(rval)
        for text, value, selected in self.options:
            if selected:
        ...
        rval.insert( cur_spot, '<select name="%s%s"%s%s%s%s%s%s>' % \
                     ( prefix, self.name+'_left', multiple, size, self.refresh_on_change_text, interaction_type[:-1]+'1"', last_selected_value, self.get_disabled_str( disabled ) ) )
        rval.append( '</select>' )
        rval.append( '</td>' )
        rval.append('<td style="float:center vertical-align:middle">')
        rval.append('<input type="button" value="--&gt;"')
        rval.append('onclick="copyOptions(\''+prefix+self.name+'_left\', \''+prefix+self.name+'\');" /><br />')
        rval.append('<input type="button" value="&lt;--"')
        rval.append('onclick="removeOptions(\''+prefix+self.name+'\', \''+prefix+self.name+'_left\');" />')
        rval.append('</td>')
        rval.append('<td style="vertical-align:top">')
        rval.append('<select name="%s%s"%s%s%s%s%s>' % \
                     ( prefix, self.name, multiple, size, self.refresh_on_change_text, interaction_type[:-1]+'2"', self.get_disabled_str( disabled ) ) )
        rval.append( '</select>' )
        rval.append('</td>')
        rval.append('</tr>')
        rval.append('</table>')
        return "\n".join( rval )
    def get_html_special( self, prefix="", disabled=False ):
        if self.multiple:
            multiple = " multiple"
        else:
            multiple = ""
        ...
        rval = []
        interaction_type =''
	display_sections=self.display.split(',')
	display_params={}
	for d in display_sections:
            display_params[d.split(':')[0]]=d.split(':')[-1]
        if 'ajax' in display_params:
            ajax_params={'table_id':display_params['ajax']}
            ajax_url=url_for( controller='requests_common', action='ret_tool_data_table_json', **ajax_params)
            interaction_type += ' ajax_url= "'+ajax_url+'"'
        else:
	    last_selected_value = ""
	    for text, value, selected in self.options:
		if selected:
	    ...
	    if last_selected_value:
		last_selected_value = ' last_selected_value="%s"' % escape( str( last_selected_value ), quote=True )
        if 'coordinate' in display_params:
            interaction_type += ' coordinate= "'+display_params['coordinate']+'"'
        last_selected_value = ""
        rval.insert( 0, '<select name="%s%s"%s%s%s%s%s%s>' % \
                     ( prefix, self.name, multiple, size, self.refresh_on_change_text, interaction_type, last_selected_value, self.get_disabled_str( disabled ) ) )
        rval.append( '</select>' )
        return unicodify( "\n".join( rval ) )
...
class ShellSelectField (SelectField):
    """
    A field which returns a div for non-standard galaxy display types
    """
    def __init__( self, name, multiple=None, display=None, refresh_on_change=False, refresh_on_change_values=None, size=None ):
        self.name = name
        self.multiple = multiple or False
        ...
        if display == "checkboxes":
            ...
            assert not( multiple ), "Radio display only supported for single select"
        elif display =="on_deck" or display == "search_select":
            pass
    ...
    def get_html( self, prefix="", disabled=False ):
        if self.display == "checkboxes":
            ...
            return self.get_html_deck( prefix, disabled )
        elif self.display == "search_select":
            return self.get_html_search_select( prefix, disabled )
        else:
            return self.get_html_default( prefix, disabled )
    def get_html_search_select( self, prefix="", disabled=False ):
        ## the ids of the div need to be set for retrieval by on submit javascript
        rval = []
        rval.append( '<div id="%s"> </div>' % \
                      ( prefix+self.name+"|search_select") )
        rval.append( '<div id="%s"> Selected: </div>' % \
                      ( prefix+self.name+"|selected") )
        rval.append( '<input type="hidden" id="%s" name="%s" value="" />' % \
