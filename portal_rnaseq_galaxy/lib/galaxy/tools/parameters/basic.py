...
class ShellSelectToolParameter ( SelectToolParameter):
    """Selection Parmeter that is specified by non-standard interaction mechanism"""
    def file_fields_to_json_string( self ):
        use_cols=set(['name','value'])
        #convert the contents of the file fields to json_contents
        fields=None
        columns=None
        source=None
        if(self.options.file_fields):
            fields=self.options.file_fields
            columns=self.options.columns
            source=self.options.index_file
        elif(self.options.tool_data_table.data):
            fields=self.options.tool_data_table.data
            columns=self.options.tool_data_table.columns
            source=self.options.tool_data_table.name
        assert source, "No index file to get json from"
        assert fields, "No file fields from: "+source
        result=[]
        for row in fields:
            result.append({})
            for col_name, position in columns.items():
                if(col_name in use_cols):
                    result[-1][col_name]=row[position]
        return simplejson.dumps(result)
    def get_data_string( self ):
        #this function should check multiple conditions and pass back the datastring depending on format required and data sources
        return self.file_fields_to_json_string()
    def get_html_field( self, trans=None, value=None, context={} ):
        # Dynamic options are not yet supported in workflow, allow 
        # specifying the value as text for now.
        if self.need_late_validation( trans, context ):
        ...
        if value is not None:
            if not isinstance( value, list ): value = [ value ]
        field = form_builder.ShellSelectField( self.name, self.multiple, self.display, self.refresh_on_change, refresh_on_change_values = self.refresh_on_change_values )
        ##no need to pass options into the field
        options = self.get_options( trans, context )
        for text, optval, selected in options:
            if isinstance( optval, UnvalidatedValue ):
                optval = optval.value
    ...
    def get_html_field( self, trans=None, value=None, other_values={} ):
        ...
        def dataset_collector( hdas, parent_hid ):
            ...
            for i, hda in enumerate( hdas ):
                if len( hda.name ) > 30:
                    hda_name = '%s..%s' % ( hda.name[:11], hda.name[-17:] )
                else:
                    hda_name = hda.name
...
parameter_types = dict( text            = TextToolParameter,
                        ...
                        shell_select    = ShellSelectToolParameter,
