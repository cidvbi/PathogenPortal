...
class InputDataModule( WorkflowModule ):
    ...
    def get_runtime_inputs( self, filter_set=['data'], type_display=None ):
        label = self.state.get( "name", "Input Dataset" )
        return dict( input=DataToolParameter( None, Element( "param", name="input", label=label, multiple=True, display=type_display, type="data", format=', '.join(filter_set) ), self.trans ) )
    def get_runtime_state( self ):
