...
class User( BaseUIController, UsesFormDefinitionsMixin ):
        ...
        if not openid_url and openid_provider == trans.app.openid_providers.NO_PROVIDER_ID:
            ...
            if request is not None:
                ...
                if request.shouldSendRedirect():
                    ...
                    form = request.htmlMarkup( trans.request.base, process_url, form_tag_attrs={'id':'openid_message','target':'_parent'} )
                    trans.app.openid_manager.persist_session( trans, consumer )
                    return form
    ...
    def __validate_login( self, trans, **kwd ):
        ...
        if not user:
            message = "Email address or password is not recognized. If you don't have an account you can register <a href=\"" +web.url_for( action='create', cntrller='user') + "\">here</a>."
            status = 'error'
        elif user.deleted:
            ...
            if trans.webapp.name == 'galaxy':
                ...
                message = 'You are now logged in as %s.<br>You can <a target="_parent" href="%s">go back to the page you were visiting</a> or <a target="_parent" href="%s">go to the home page</a>.' % \
                    ( user.email, redirect, url_for( '/' ) )
                if trans.app.config.require_login:
                    message += '  <a target="_parent" href="%s">Click here</a> to continue to the home page.' % web.url_for( '/static/welcome.html' )
            success = True
    ...
    def logout( self, trans, logout_all=False ):
        ...
        message = 'You have been logged out.<br>You can log in again, <a target="_parent" href="%s">go back to the page you were visiting</a> or <a target="_parent" href="%s">go to the home page</a>.' % \
    ...
    def __register( self, trans, cntrller, subscribe_checked, **kwd ):
        ...
        if error:
            ...
            message = 'Now logged in as %s.<br><a target="_parent" href="%s">Return to the home page.</a>' % ( user.email, url_for( '/' ) )
            message = 'Now logged in as %s.<br>Go to the <a target="_parent" href="%s">Launch Pad</a> to begin using the RNA-Seq Pipeline.' % ( user.email, url_for( '/launch_pad/index' ) )
            success = True
        return ( message, status, user, success )
