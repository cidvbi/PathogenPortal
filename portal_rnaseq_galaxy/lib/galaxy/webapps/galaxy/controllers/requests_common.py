...
class RequestsCommon( BaseUIController, UsesFormDefinitionsMixin ):
    @web.json
    def ret_tool_data_table_json(self, trans, table_id, use_cols=None):
        if not use_cols:
        ...
        columns=None
        cur_table=None
        cur_table= trans.app.tool_data_tables[table_id]
        assert cur_table, "Cannot retrieve table with ID "+table_id
        fields=cur_table.get_fields()
        assert fields, "No file fields from: "+table_id
        columns=cur_table.columns
        rval=[]
        for row in fields:
            rval.append({})
            for col_name, position in columns.items():
                if(col_name in use_cols):
                    rval[-1][col_name]=row[position]
        return rval        
    @web.json
