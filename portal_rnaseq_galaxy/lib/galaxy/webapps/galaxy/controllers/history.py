...
class HistoryListGrid( grids.Grid ):
    ...
    class DeletedColumn( grids.DeletedColumn ):
        def get_value( self, trans, grid, history ):
            if history == trans.history:
                return "<strong>current project</strong>"
            if history.purged:
    ...
    columns = [
        HistoryListNameColumn( "Project Name", key="name", attach_popup=True, filterable="advanced" ),
        ...
        for i in range(len(histories)):
            if histories[i].user_id == user.id:
                if name[i] == histories[i].get_display_name():
                    change_msg = change_msg + "<p>Project: "+cur_names[i]+" is already named: "+name[i]+"</p>"
                elif name[i] not in [None,'',' ']:
                    name[i] = escape(name[i])
                    histories[i].name = sanitize_html( name[i] )
                    trans.sa_session.add( histories[i] )
                    trans.sa_session.flush()
                    change_msg = change_msg + "<p>Project: "+cur_names[i]+" renamed to: "+name[i]+ "</p>"  
                    change_msg = change_msg + "Return to <a target=\"_top\" href=\"" + (url_for('/pp_status')) + "\"> Project List</a>"
                    change_msg = change_msg + "<script type='text/javascript'>window.top.location=\"" + (url_for('/pp_status')) + "\";</script>"
                    trans.log_event( "Project renamed: id: %s, renamed to: '%s'" % (str(histories[i].id), name[i] ) )
                else:
                    change_msg = change_msg + "<p>You must specify a valid name for Project: "+cur_names[i]+"</p>"
            else:
                change_msg = change_msg + "<p>Project: "+cur_names[i]+" does not appear to belong to you.</p>"
