...
class WorkflowController( BaseUIController, SharableMixin, UsesStoredWorkflowMixin, UsesAnnotations, UsesItemRatings ):
    ...
    def get_universal( self, trans ):        
        universal_share= trans.sa_session \
            .query( model.StoredWorkflowUserShareAssociation ) \
            .join( 'stored_workflow' ) \
            .filter( model.StoredWorkflow.deleted == False ) \
            .filter( model.StoredWorkflowUserShareAssociation.user_id==model.StoredWorkflow.user_id ) \
            .order_by( desc( model.StoredWorkflow.update_time ) ) \
            .all()
        return universal_share
    ...
    def list( self, trans ):
        ...
        slug_set = False
        for workflow_assoc in universal_share:
        ...
        return trans.fill_template( "workflow/list.mako",
                                    workflows = workflows,
                                    shared_by_others = universal_share )
    ...
    def share( self, trans, id, email="", use_panels=False ):
        ...
        if email:
            ...
            elif other == trans.get_user() and not trans.user_is_gateway(trans.get_user()):
                mtype = "error"
    ...
    @web.expose
    def run( self, trans, id, history_id=None,  multiple_input_mode="product", hide_fixed_params=False, start_mako="workflow/run.mako", finish_mako="workflow/run_complete.mako", **kwargs ):
        stored = self.get_stored_workflow( trans, id, check_ownership=False )
        user = trans.get_user()
        if stored.user != user:
            if trans.sa_session.query( model.StoredWorkflowUserShareAssociation ) \
                    .filter_by( user=user, stored_workflow=stored ).count() == 0 \
		    and not (trans.user_is_gateway(stored.user) and \
		    trans.sa_session.query( model.StoredWorkflowUserShareAssociation )\
		    .filter_by( user_id=stored.user_id, stored_workflow=stored ).count() > 0) :
                error( "Workflow is not owned by or shared with current user" )
        ...
        try: # use a try/finally block to restore the user's current history
            if kwargs:
                ...
                multiple_keyarray = []
                multiple_inputs = {}
                #keep track of the input slot with the minimum number of incoming objects
                minput=1
                for input_key in input_keys:
                    if isinstance(kwargs[input_key], list):
                        if(minput==1):
                            minput=len(kwargs[input_key])
                        else:
                            minput=min(minput,len(kwargs[input_key]))
                        multiple_keyarray.append(input_key)
                        multiple_inputs[input_key]= kwargs[input_key]
                # List to gather values for the template
                invocations=[]
                for input_number in range(0,minput):
                    # Example: single_input='1', single_input='2', etc...
                    # 'Fix' the kwargs, to have only the input for this iteration
                    for cur_key in multiple_keyarray:
                        single_input=multiple_inputs[cur_key][input_number]
                        kwargs[cur_key]=single_input
                    for step in workflow.steps:
                    ...
                    if 'run_workflow' in kwargs and not errors:
                        ...
                        trans.sa_session.flush()
                if invocations:
                    return trans.fill_template( finish_mako,
                                                    workflow=stored,
                ...
                if missing_tools:
                    ...
                    return trans.fill_template(start_mako, steps=[], workflow=stored, hide_fixed_params=hide_fixed_params, missing_tools = missing_tools)
            # Render the form
            stored.annotation = self.get_item_annotation_str( trans.sa_session, trans.user, stored )
            return trans.fill_template(
                        start_mako,
                        ...
                        enable_unique_defaults=trans.app.config.enable_unique_workflow_defaults, display_elements={})
        finally:
