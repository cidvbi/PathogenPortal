...
/**
 *  * Synchronize selections on page.
 *   */
function sync_select(coord_key, cur_select) {
    var select_list=$('[coordinate="'+coord_key+'"]');
    select_list.each( function() {
    	var to_sync= $(this);
	if (to_sync != cur_select.currentTarget){
		to_sync.select2("val",cur_select.val);
	}
    });
}
...
function replace_big_select_inputs(min_length, max_length, select_elts) {
    ...
    select_elts.each( function() {
        count=count+1;
        var select_elt = $(this);
	var ajax_url =select_elt.attr('ajax_url');
	...
	var coordinate =select_elt.attr('coordinate');
	if ( coordinate != undefined){
	    select_elt.attr('id',coordinate+count);
	}
	select_elt.select2( { width: "400px" } )
	if (ajax_url != undefined) {
		$.getJSON(ajax_url, function(data){
		    ...
		    select_elt.append(html);
		    select_elt.select2("val",select_elt.find('option:eq(0)').attr('value'));
		});
	}
	if ( coordinate != undefined){
	    $("#"+coordinate+count).on("change", function(e) {
	    	sync_select(coordinate, e);
	    });
	}

    });
    /*var input_elts = $('input');

    input_elts.each (function () {
    	var input_elt = $(this);
	var ajax_url =input_elt.attr('ajax_url');
...
$(document).ready( function() {
    ...
    // Replace big selects.
    replace_big_select_inputs(20, 15000);
