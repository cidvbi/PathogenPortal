...
def merge_callback( option, op_str, value, parser ):
    merge_mode = []
    for arg in parser.rargs:
        arg = arg.strip()
        if arg[0] is "-":
            ...
            merge_mode.append(arg)

    setattr(parser.values, option.dest, merge_mode)    
...
def __main__():
    ...
    parser.add_option( "--merge_mode", dest="merge_mode", action="callback", callback=merge_callback, help="GTF files for merging.")
    # Cuffdiff options.
    ...
    # Outputs.
    parser.add_option( "--merged_transcripts", dest="merged_transcripts" )
    ...
    if options.merge_mode:
        # Base.
        merge_cmd = "cuffmerge -o cm_output "
        # Add options.
        if options.num_threads:
            merge_cmd += ( " -p %i " % int ( options.num_threads ) )
        if options.ref_annotation:
            merge_cmd += " -g %s " % options.ref_annotation
        #if options.use_seq_data:
        #    merge_cmd += " -s %s " % seq_path
            
        # Add input files to a file.
        inputs_file_name = tempfile.NamedTemporaryFile( dir="." ).name
        inputs_file = open( inputs_file_name, 'w' )
        for gtf in options.merge_mode:
            inputs_file.write( gtf + "\n" )
        inputs_file.close()
        merge_cmd += inputs_file_name
        ...
        try:        
            ...
            proc = subprocess.Popen( args=merge_cmd, shell=True, stderr=tmp_stderr.fileno() )
            returncode = proc.wait()
            tmp_stderr.close()
            ...
            #get the merged GTF from the 
            options.inputA=options.merged_transcripts
