...
def __main__():
    ...
    parser.add_option( '', '--useCS', dest='useCS', help='whether to use CSFasta' )
    parser.add_option( '', '--qual', dest='qual', help='CS quality file' )
    parser.add_option( '', '--qual1', dest='qual1', help='CS quality file read 1' )
    parser.add_option( '', '--qual2', dest='qual2', help='CS quality file read 2' )
    ...
    query_option = '-q'
    if options.useCS == "true":
        query_option = '-f'
    if options.params == 'preSet':
        aligning_cmds = query_option+' %s %s -p %s -S %s %s %s ' % \
        ...
        try:
            ...
            if options.keepends and options.keepends == 'doKeepends':
                ...
                keepends = ''
            aligning_cmds = query_option+' %s %s -p %s -S %s %s %s %s %s %s %s %s %s %s %s %s ' \
    ...
    try:
        ...
        try:
            #create temporary file for the sam output of bowtie
            #instead of using options.output which is reserved for the final bam
            tmp_sam_file=tempfile.NamedTemporaryFile(dir=tmp_index_dir).name
            # prepare actual mapping commands
            if options.paired == 'paired':
                if options.useCS == 'true':
                    cmd2 = 'bowtie %s --Q1 %s --Q2 %s %s -1 %s -2 %s> %s' % ( aligning_cmds, options.qual1, options.qual2, ref_file_name, options.input1, options.input2, tmp_sam_file )
                else:
                    cmd2 = 'bowtie %s %s -1 %s -2 %s > %s' % ( aligning_cmds, ref_file_name, options.input1, options.input2, tmp_sam_file )
            else:
                if options.useCS == 'true':                
                    cmd2 = 'bowtie %s -Q %s %s %s > %s' % ( aligning_cmds, options.qual, ref_file_name, options.input1,  tmp_sam_file )
                else:
                    cmd2 = 'bowtie %s %s %s > %s' % ( aligning_cmds, ref_file_name, options.input1, tmp_sam_file )
            print "Command is: "+cmd2
            # align
            tmp = tempfile.NamedTemporaryFile( dir=tmp_index_dir ).name
            print "Error file is: "+tmp
            tmp_stderr = open( tmp, 'wb' )
            ...
            if os.path.getsize( tmp_sam_file ) == 0:
            ...
            #try running the commands for converting sam to bam
        try:
            ...
            tmp_aligns_file = tempfile.NamedTemporaryFile( dir=tmp_index_dir )
            tmp_aligns_file_name = tmp_aligns_file.name
            tmp_aligns_file.close()
            tmp_bam=tempfile.NamedTemporaryFile( dir=tmp_index_dir).name
            cmd3 = 'samtools view -S -b %s > %s' % (tmp_sam_file, tmp_bam)
            tmp = tempfile.NamedTemporaryFile( dir=tmp_index_dir ).name
            tmp_stderr = open( tmp, 'wb' )
            proc = subprocess.Popen( args=cmd3, shell=True, cwd=tmp_index_dir, stderr=tmp_stderr.fileno() )
            returncode = proc.wait()
            tmp_stderr.close()
            # get stderr, allowing for case where it's very large
            tmp_stderr = open( tmp, 'rb' )
            ...
            if os.path.exists( tmp_index_dir ):
                shutil.rmtree( tmp_index_dir )
            stop_err( 'Error converting sam to bam from (%s), %s' % ( tmp_sam_file, str( e ) ) )

        try:
            # sort the BAM file
            tmp_final_bam = tempfile.NamedTemporaryFile( dir=tmp_index_dir ).name
            cmd4 = 'samtools sort %s %s' % (tmp_bam, tmp_final_bam)
            tmp = tempfile.NamedTemporaryFile( dir=tmp_index_dir ).name
            tmp_stderr = open( tmp, 'wb' )
            proc = subprocess.Popen( args=cmd4, shell=True, cwd=tmp_index_dir, stderr=tmp_stderr.fileno() )
            returncode = proc.wait()
            tmp_stderr.close()
            # get stderr, allowing for case where it's very large
            tmp_stderr = open( tmp, 'rb' )
            ...
            try:
                ...
                pass
            tmp_stderr.close()
            sorted_bam = '%s.bam' % tmp_final_bam
            if returncode != 0:
                raise Exception, stderr
            # check that there are results in the output file
            if os.path.getsize( sorted_bam ) == 0:
                raise Exception, 'The output file is empty, there may be an error with your input file or settings.'
            #move the bam file to its final resting place
            shutil.move( sorted_bam, options.output)
        except Exception, e:
            #clean up temp files
            if os.path.exists( tmp_index_dir ):
                shutil.rmtree( tmp_index_dir )
            stop_err( 'Error sorting  bam from (%s), %s' % ( tmp_bam, str( e ) ) )
    finally:
        # clean up temp dir
        if os.path.exists( tmp_index_dir ):
            shutil.rmtree( tmp_index_dir )
