...
my $email_domain = '@vbi.vt.edu';

# mysql db
my $mysql_database = 'galaxy';
my $mysql_host = 'localhost';
my $mysql_username = "galaxy";
my $mysql_password = "";
my $sql_type='pg'; #or 'mysql'

# file path
my $db_root_dir = '/opt/hts_software/galaxy-parent/galaxy_server/galaxy-dist/database/files/';
##############################################################
...
sub get_data
{
	my $dbh;
	#if($sql_type=='mysql'){
	#	$dbh = DBI->connect("DBI:mysql:database=$mysql_database;host=" . $mysql_host, $mysql_username, $mysql_password, {'RaiseError' => 1});
	#}
	#elsif($sql_type=='pg'){
		$dbh = DBI->connect("dbi:Pg:dbname=$mysql_database",$mysql_username, $mysql_password, {'RaiseError' => 1});
	#}
	#else{
	#	die "SQL type not valid: $!\n";
	#}
	...
	print "$current_user$email_domain\n";
