...
<%def name="late_javascripts()">
    ...
    %if trans.user and tool_id is None and workflow_id is None and m_c is None:
...
<%def name="init()">
<%
    self.has_left_panel = app.config.is_admin_user( trans.user ) or app.config.is_power_user( trans.user ) 
...
<%def name="center_panel()">
    ...
    if trans.app.config.require_login and not trans.user:
        ...
        center_url = h.url_for( 'tool_runner', tool_id=tool_id)
        ...
        center_url = h.url_for( controller='launch_pad',  action='task_menu' )
    %>
...
<%def name="right_panel()">
    <div class="unified-panel-header" unselectable="on">
        <div class="unified-panel-header-inner">
            ...
            <div class="panel-header-text">${_('Current Project History')}</div>
        </div>
    </div>
    <div class="unified-panel-body" style="overflow: hidden;">
